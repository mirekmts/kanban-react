...
describe('<AddTaskForm/>', () => {
  ...
  beforeEach(() => {
    const mock = jest.fn();
    wrapper = shallow(<AddTaskForm onCreateTask={mock} hideAddTaskForm={mock} listId="2" />);
  });
  ...
  describe('updating value and add new task', () => {
    ...
    it('Prevent from adding task with name more than 30 characters', () => {
      wrapper
        .find('Input')
        .at(0)
        .simulate('change', { target: { value: 'this is a very long name more than 30 characters', name: 'name' } });
      wrapper.find('Form').simulate('submit', { preventDefault: () => {} });
